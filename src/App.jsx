...
export default function App() {
  ...
  function issue(form) { run(async () => {
    ...
    if (mode === 'mocknet') {
      ...
      setNotice(`${id} issued on Mocknet: ${transaction.txid.slice(0, 14)}…`);
      return;
    }
  ...
  function renew(license, years) { run(async () => {
    if (mode === 'mocknet') {
      ...
      setNotice(`${license.id} renewed on Mocknet: ${transaction.txid.slice(0, 14)}…`);
      return;
    }
    await new Promise((resolve) => setTimeout(resolve, 350));
  ...
  function revoke(license) { run(async () => {
    if (mode === 'mocknet') {
      ...
      setNotice(`${license.id} revoked on Mocknet: ${transaction.txid.slice(0, 14)}…`);
      return;
    }
    await new Promise((resolve) => setTimeout(resolve, 350));
