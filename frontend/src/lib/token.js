/**
 * Mocknet license commitment helpers.
 *
...
// The commitment uses a four-byte little-endian expiry. Keep dates below 2038.
export function uint32ToScriptNumberHex(value) {
  ...
  let remaining = value;
  do {
    encoded.push(remaining % 256);
    remaining = Math.floor(remaining / 256);
  } while (remaining > 0);
  // Positive script numbers reserve the high bit for the sign. The configured
  // maximum never needs this extra byte, but retaining the rule keeps the
  // encoding correct if the allowed range changes later.
  if (encoded.at(-1) & 0x80) encoded.push(0);
